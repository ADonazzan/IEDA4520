...
# Importing data
df2 = Data.GetData(start_date, df_end, trade_days, Update)
df = pd.DataFrame(df2)
...
def estimate_prices(i):
    S0 = ns.df.iloc[i].S0       # Sets S0
    K = ns.df.iloc[i].strike
...
if __name__ == '__main__':
    ...
    for _ in tqdm(pool.imap_unordered(estimate_prices, range(len(df))), total=len(df)):
        pass
    pool.close()
    pool.join()
